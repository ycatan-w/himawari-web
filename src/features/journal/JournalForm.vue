...
import { IconCircleCloseOutline, IconCircleCloseSolid, IconEmoji, IconFlower, IconHead, IconList, IconPublish } from '@/components/ui/icons';
...
<template>
  <div v-if="entryToEdit.id" class="flex items-center gap-2 px-3 py-2 bg-white/10 border-b border-white/10">
  <button
    @click="onEscape"
    type="button"
...
<form>
  <div class="relative mt-5">
      ...
      >
        <span class="relative z-10 text-white/80 md:text-[currentColor] group-hover:text-white/80 flex items-center justify-center gap-2">
          <IconPublish />
        </span>
        <span
          :class="[
